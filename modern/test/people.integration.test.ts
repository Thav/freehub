...
test("people preserve display contacts, normalize match fields, archive reversibly, and remain tenant-scoped", { skip: !db }, async () => {
  ...
  const archived = await app.inject({ method: "GET", url: `/api/organizations/${organization.id}/people/archived?q=Pat`, headers: { cookie: auth.cookie } }); assert.equal(archived.statusCode, 200); assert.ok((archived.json() as { id: string }[]).some((person) => person.id === personId));
...
test("tags are organization-owned and notes aggregate person, visit, and service targets with audit attribution", { skip: !db }, async () => {
  ...
  const tagged = await app.inject({ method: "GET", url: `/api/organizations/${organization.id}/tags/${volunteer.id}`, headers: { cookie: auth.cookie } }); assert.equal(tagged.statusCode, 200); const taggedPerson = (tagged.json() as { people: { id: string; membershipEndsOn: string | null; lastVisitAt: string | null }[] }).people[0]; assert.equal(taggedPerson.id, person.id.toString()); assert.equal(taggedPerson.membershipEndsOn, null); assert.ok(taggedPerson.lastVisitAt);
  ...
  for (const response of [personNote, visitNote, serviceNote]) assert.equal(response.statusCode, 201); const personNoteBody = personNote.json() as { createdByUserId: string; createdByName: string }; assert.equal(personNoteBody.createdByUserId, "1"); assert.equal(personNoteBody.createdByName, "Spike Manager");
  const aggregate = await app.inject({ method: "GET", url: `/api/organizations/${organization.id}/people/${person.id}/notes`, headers: { cookie: auth.cookie } }); assert.equal(aggregate.statusCode, 200); const aggregateNotes = aggregate.json() as { text: string; createdByName: string }[]; assert.deepEqual(new Set(aggregateNotes.map((note) => note.text)), new Set(["person note", "visit note", "service note"])); assert.deepEqual(new Set(aggregateNotes.map((note) => note.createdByName)), new Set(["Spike Manager"]));
